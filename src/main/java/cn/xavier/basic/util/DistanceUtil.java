...
public class DistanceUtil {
    public static Point getPoint(String address) {
        String Application_ID = "vopl8kM8PtS3S9zEzcLFg6j8hYx89hO3";//配置上自己的百度地图应用的AK
        try {
            String sCurrentLine;
            String sTotalString;
            sCurrentLine = "";
...
//          URL l_url = new java.net.URL("http://api.map.baidu.com/geocoder/v2/?address="+address.replaceAll(" ","")
// +"&output=json&ak="+Application_ID+"&callback=showLocation");
            URL l_url = new URL("http://api.map.baidu.com/geocoding/v3/?address=" + address + "&output=json&ak=" +
                    Application_ID + "&callback=showLocation");
    ...
    public static Shop getNearestShop(Point point, List<Shop> shops) {
        ...
        if (shopSize > 1) {
            for (int i = 1; i < shopSize; i++) {
                double distanceTemp = getDistance(point, getPoint(shops.get(i).getAddress()));
